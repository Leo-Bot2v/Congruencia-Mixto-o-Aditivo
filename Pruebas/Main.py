...
from static.Backend import Backend
from PySide6.QtWebChannel import QWebChannel
from static.Interceptor import Interceptor
...
class MiNavegador(QWidget):
    def __init__(self):
        ...
        self.interceptor = Interceptor(self.webview)
        ...
        self.setMinimumSize(1366, 715)
        self.showMaximized()

        self.webview.loadFinished.connect(self.recargar_pagina)
        self.reloaded = False

    def recargar_pagina(self, ok):
        if ok and not self.reloaded:
            self.reloaded = True
            self.webview.reload()
